...
<article class="container">
  ...
  <p><strong>What the dataset is and is not.</strong> This is a benchmark. We do <em>not</em> observe real shopper traffic — actual shopping interactions with ChatGPT, Claude, Perplexity etc. are private to each agent and not publicly accessible. We construct a query suite designed to test agent behavior at the constraint-rich end of buyer intent (specific verticals, factual attributes, price tiers), then run it daily. The signal the dataset surfaces is "given a buyer-style query, which catalogs do AI agents cite?" — useful for benchmarking visibility and tracking changes over time. The signal it does <em>not</em> surface is "what queries real shoppers type to AI agents and at what volume" — that data exists only inside each agent's servers.</p>

  <p>Limitations:</p>
  <ul>
    <li>The query suite is synthetic, not sampled from real search logs. Query phrasings lean longer and more constraint-loaded than typical shopper phrasing.</li>
    <li>Query volumes are roughly uniform across the suite (each query run a similar number of times daily), not weighted by realistic shopper attention.</li>
